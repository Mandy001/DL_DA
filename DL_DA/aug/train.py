...
# Create the generator
    ...
    fixed_noise = torch.randn(64, cf.nz, device=device)
    ...
    for epoch in range(cf.num_epochs):
        ...
        for i, data in enumerate(il.data_loader(l), 0):
            ...
            noise = torch.randn(b_size, cf.nz, device=device)
            ...
            if i % 50 == 0:
                print('[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f\tD(x): %.4f\tD(G(z)): %.4f / %.4f'
                      % (epoch, cf.num_epochs, i, len(il.data_loader(l)),
                ...
                if (iters % 500 == 0) or ((epoch == cf.num_epochs-1) and (i == len(il.data_loader(l))-1)):
                    with torch.no_grad():
                        fake = netG(fixed_noise).detach().cpu()
                        img_list.append(vutils.make_grid(fake, padding=2, normalize=True))
                        
                        iters += 1
    torch.save(netG,"netTG_{}.pt".format(l))
