...
from sklearn.metrics import precision_recall_fscore_support, classification_report, confusion_matrix, accuracy_score,f1_score,roc_auc_score
...
def create_CNN_model(num_classes):
    model = keras.Sequential([
        keras.layers.Lambda(lambda x: tf.expand_dims(x, axis=-1)),
        keras.layers.Conv1D(120, 5, activation=tf.nn.relu),
        keras.layers.Lambda(lambda x: tf.reduce_max(x, axis=-1)),
        # keras.layers.Flatten(),
        keras.layers.Dense(500, activation=tf.nn.relu),
        keras.layers.Dense(num_classes, activation=keras.activations.softmax)
...
def train_and_predict(create_model_func, X, Y, train_indices, test_indices, split_index=None):
    ...
    X_train = np.vstack((X[train_indices], new_X))

    Y_train = tf.one_hot(np.hstack((Y[train_indices], new_Y)), depth=num_classes).numpy()
